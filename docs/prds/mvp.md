...
**Approach:** Automated tests cover core logic that's straightforward to verify in isolation — pane layout tree operations (split, resize, close), session state persistence and restore, attention-state transitions, and worktree creation/cleanup logic. Manual exploratory testing covers the interactive UX — pane splitting and resizing by hand, the focus/expand interaction, choosing a working directory and toggling worktree creation at pane-creation time, and the end-to-end permission dialog flow — run against real local Claude Code sessions.
