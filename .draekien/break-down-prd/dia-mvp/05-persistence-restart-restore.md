...
- [x] **T5** [AFK] Resume-on-focus trigger: `FocusPane` already in `IpcCommand` union, `DiaApi.focusPane`, preload bridge, and gateway dispatch (`FocusPane` → `PaneWorkspace.resumePane`); renderer already wires `onFocusPane` in `app.tsx` (`useCallback` → `window.dia.focusPane`). `PaneWorkspace.resumePane` idempotent (no-op when the pane has a live handle, is unknown, or has no recorded `sessionId`), degrades to usable pane (emits `Errored` attention for gone non-worktree cwd or spawn/reattach failure). Threads `resume: sessionId` through `PaneSupervisor.openPane` → `startProcess` → `Init` message → `agent-session.ts` → `query()`. Tests: supervisor Init message contains resume; workspace resumePane idempotent, no-op conditions, error handling; gateway FocusPane dispatch. — serves: US-10 — depends: T3, T6
- [x] **T6** [AFK] Worktree reattach: `GitOpsService.reattachWorktree` implemented (+ `WorktreeReattachError`) running `git worktree add <path> dia/<paneId>` (checks out existing branch — no `-b`, never `-B`, preserves committed work). Guarded via `FileSystem.exists` check: already-present → logs warning and no-ops; not-present → reattaches. Returns same `WorktreeInfo`. Tests: ✓ reattach uses exactly `worktree add <path> <branch>` (asserts no `-b`/`-B`); ✓ non-zero exit → `WorktreeReattachError`; ✓ existing path → no git command, log warning. — serves: US-10 — depends: T0
