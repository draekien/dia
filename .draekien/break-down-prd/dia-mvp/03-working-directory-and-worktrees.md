...
- [ ] **T2** [AFK] Implement `GitOpsService.createWorktree`/`.removeWorktree` via `@effect/platform`'s `Command` executor (`git worktree add`/`remove`), returning typed `WorktreeCreateError`/`WorktreeRemoveError` (§4.6). Named `GitOpsService` rather than a worktree-only service since more git operations are expected in later bullets — serves: US-5, US-6 — depends: T1
- [ ] **T3** [AFK] Automated tests for `GitOpsService` against a test `Layer` in place of the real git executor: successful create/remove, and remove-failure-is-logged-not-thrown — serves: US-6, G-5 — depends: T2
