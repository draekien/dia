...
- **`main` level with origin before bumping.** Push any outstanding commits
  first, so the only new commit on the remote after this procedure is the
  version-bump commit the tag will point at.
...
1. **Choose the bump level.** If the argument names a level, use it. Otherwise
   inspect the commits since the last release tag —
   `git log $(git describe --tags --abbrev=0)..HEAD --oneline` — and choose:
   `patch` for fixes and chores, `minor` for a notable feature set, `major` only
   on the user's explicit call. Default to `patch` when unsure. State your choice
   and what drove it.
...
The top row must show the new tag with an `in_progress` (or queued) status. If
no run for the tag appears within a minute, do not re-push: confirm the tag
reached origin (`git ls-remote --tags origin`) and carries its `v` prefix, then
surface the missing run to the user rather than assuming the release started.
