...
describe('GitOpsService', () => {
  ...
  it.effect(
    'removeWorktree fails with WorktreeRemoveError on non-zero exit, without throwing',
    () =>
      Effect.gen(function* () {
        const { testLayer, loggerLayer } = makeTestSetup(() => 1)
        const info: WorktreeInfo = {
        ...
        const result = yield* Effect.gen(function* () {
          const gitOps = yield* GitOpsService
          return yield* gitOps.removeWorktree(info, PANE_ID)
        }).pipe(Effect.provide(testLayer), Effect.provide(loggerLayer), Effect.either)

        assert.isTrue(Either.isLeft(result))
        if (Either.isLeft(result)) {
          assert.instanceOf(result.left, WorktreeRemoveError)
        }
      })
  )
})
